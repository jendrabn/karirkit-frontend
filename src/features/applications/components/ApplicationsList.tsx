...
import { ColumnToggle, defaultColumnVisibility, type ColumnVisibility } from "./ColumnToggle";
import { ApplicationStats } from "./ApplicationStats";
import { useApplications, type GetApplicationsParams } from "../api/get-applications";
...
import { type Application, type JobType, type WorkSystem, type ApplicationStatus, type ResultStatus, JOB_TYPE_OPTIONS, WORK_SYSTEM_OPTIONS, STATUS_OPTIONS, RESULT_STATUS_OPTIONS } from "@/types/application";
...
export const ApplicationsList = () => {
    ...
    const [filters, setFilters] = useState<Omit<GetApplicationsParams, "page" | "per_page" | "q" | "sort_by" | "sort_order">>({});
    const [columnVisibility, setColumnVisibility] = useState<ColumnVisibility>(defaultColumnVisibility);

    const [sortField, setSortField] = useState<GetApplicationsParams["sort_by"]>("date");
    ...
    const { data: applicationsData, isLoading } = useApplications({ params: queryParams });
    ...
    const EditableCell = ({
        ...
        const handleBlur = () => {
            setIsEditing(false);
            if (value !== app[field]) {
               const { id, user_id, created_at, updated_at, ...data } = { ...app, [field]: value };
        ...
        const handleSelectChange = (val: string) => {
             const { id, user_id, created_at, updated_at, ...data } = { ...app, [field]: val };
             updateApplicationMutation.mutate({
                   id: app.id,
                   data: data as any
               });
        ...
        if (type === "select") {
            ...
            let getBadgeVariant: () => string = () => "default";
             ...
             if (field === "job_type") { options = JOB_TYPE_OPTIONS; getBadgeVariant = () => getJobTypeBadgeVariant(app.job_type);}
             else if (field === "work_system") { options = WORK_SYSTEM_OPTIONS; getBadgeVariant = () => getWorkSystemBadgeVariant(app.work_system);}
             else if (field === "status") { options = STATUS_OPTIONS; getBadgeVariant = () => getStatusBadgeVariant(app.status);}
             else if (field === "result_status") { options = RESULT_STATUS_OPTIONS; getBadgeVariant = () => getResultStatusBadgeVariant(app.result_status);}
            ...
            return (
                ...
                >
                  <SelectTrigger className="h-auto w-full border-0 bg-transparent p-0 shadow-none focus:ring-0">
                    ...
                    >
                      {options.find((opt) => opt.value === app[field])?.label || app[field]}
                    </Badge>
                  </SelectTrigger>
          ...
          <Table>
            ...
            <TableBody>
                ...
                applications.map((app, index) => (
                    <TableRow 
                        key={app.id} 
                        className={cn(index % 2 === 1 && "bg-muted/30", selectedIds.includes(app.id) && "bg-primary/5")}
                    >
                         <TableCell>
                          <Checkbox
